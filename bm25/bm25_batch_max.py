...
- bm25 인덱스 폴더 안에 이미 다음 파일들이 있어야 한다.
    ...
    - bm25_docs.json
    - bm25_chunk2doc.json
- 토크나이저는 simple 또는 okt 사용 (기본: okt)
"""
...
def main():
    ...
    ap.add_argument(
        "--save_dir",
        required=True,
        help="BM25 인덱스 폴더 (bm25_matrix.npz, bm25_vocab.json, bm25_docs.json 이 있는 곳)",
    )
    ap.add_argument(
        "--chunk2doc",
        required=True,
        help="prepare-delimited 시 생성된 bm25_chunk2doc.json 경로",
    )
    ap.add_argument(
        "--tokenizer",
        choices=["simple", "okt"],
        default="okt",
        help="BM25 인덱스에서 사용할 토크나이저 (기본: okt 명사 추출)",
    )
    ap.add_argument(
        "--mode",
    ...
    ap.add_argument(
        ...
        type=int,
        default=4,
        help="mode=topMsum 일 때 상위 몇 개 chunk score를 더할지",
    )
    ap.add_argument(
        "--topk",
        type=int,
    ...
    for f in files:
        ...
        ranked = bm25_doc_similarity_from_chunks(
            ...
            tokenizer=args.tokenizer,  # ← 여기서 'okt'면 bm25.py의 _tokenize_okt_nouns 사용
            mode=args.mode,
            M=args.M,
            topk=args.topk,
        )
