...
def generate_with_ollama(model: str, prompt: str) -> str:
    """
    1) HTTP API (http://localhost:11434/api/generate) 먼저 시도
    2) 실패 시 `ollama run 모델` CLI 폴백
    """
    # 1) HTTP 시도
    try:
        import requests
        ...
        print(f"[정보] Ollama HTTP 호출 실패, CLI로 폴백합니다: {e}")
...
def main():
    parser = argparse.ArgumentParser(description="RAG Pipeline (Hybrid + Ollama 기밀 유출 리포트)")
    ...
    p_query.add_argument(
        ...
        type=str,
        default="",
        help="Ollama 요약 마크다운 저장 경로",
    )
    p_query.add_argument(
        "--ollama_model",
        type=str,
        default="llama3.1",
        help="Ollama 모델 이름 (예: llama3.1, qwen2.5 등)",
    )
    ...
    if args.command == "query":
        ...
        if not top3:
            print("[경고] 상위 문서가 없어 Ollama 리포트를 생성하지 않습니다.")
            return
        ...
        if not internal_text_map:
            print("[경고] 내부 문서 텍스트가 없어 Ollama 리포트를 건너뜁니다.")
            return

        # 텍스트 truncate
        ext_text_trunc = truncate_text(query_text)

        # 프롬프트 헤더 (원하는 리포트 형식 반영)
        prompt_header = f"""
...
"""
        ...
        if not llm_output:
            print("[경고] Ollama에서 결과를 받지 못했습니다.")
            return
        ...
        if args.out_json:
            try:
                with open(args.out_json, "w", encoding="utf-8") as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                print(f"[저장] Ollama 요약 포함 전체 결과 JSON 업데이트: {args.out_json}")
            except Exception as e:
        ...
        if args.out_summary:
            try:
                with open(args.out_summary, "w", encoding="utf-8") as f:
                    f.write(llm_output)
                print(f"[저장] Ollama 마크다운 요약 파일: {args.out_summary}")
            except Exception as e:
                print(f"[경고] 요약 파일 저장 실패: {e}")

        # 콘솔에 최종 리포트 출력
        print("\n======================= Ollama 유사도/기밀 분석 리포트 =======================\n")
        print(llm_output)
        print("\n===========================================================================")
