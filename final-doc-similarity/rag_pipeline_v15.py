...
try:
    ...
    print("[오류] openai 패키지를 찾을 수 없습니다. 'pip install openai' 후 다시 시도하세요.")
    sys.exit(1)
...
def generate_with_gpt(model: str, prompt: str) -> str:
    """
    OpenAI Responses API를 사용해 GPT 모델 호출.
    ...
    try:
        ...
        # 최신 openai-python에서는 output_text 바로 사용 가능
...
def main():
    parser = argparse.ArgumentParser(description="RAG Pipeline (Hybrid + GPT 기밀 유출 리포트)")
    ...
    p_query.add_argument(
        ...
        type=str,
        default="",
        help="GPT 요약 마크다운 저장 경로",
    )
    p_query.add_argument(
        "--gpt_model",
        type=str,
        default="gpt-4.1-mini",
        help="사용할 GPT 모델 이름 (예: gpt-4.1-mini, gpt-4.1 등)",
    )
    ...
    if args.command == "query":
        ...
        if not top3:
            print("[경고] 상위 문서가 없어 GPT 리포트를 생성하지 않습니다.")
            return
        ...
        if not internal_text_map:
            print("[경고] 내부 문서 텍스트가 없어 GPT 리포트를 건너뜁니다.")
            return

        # 외부/내부 텍스트 truncate
        ext_text_trunc = truncate_text(query_text)

        # 프롬프트 헤더 (원하는 출력 예시 스타일 반영)
        prompt_header = f"""
...
- "유사 문장 예시"에 쓰는 문장은 반드시 내가 아래에 제공하는 텍스트에 실제로 들어있는 문장을 그대로 인용해야 한다.
- 텍스트에 없는 구체적인 숫자, 지명, 인물, 리스크 내용을 새로 지어내지 말고, 보이지 않으면 "구체적인 수치는 잘리지 않아 보이지 않는다"처럼 설명하라.

출력 형식은 아래 예시와 최대한 비슷하게 맞춰라 (제목만 바뀌고, 내용은 네가 분석해서 채운다):
...
각 문서의 내용 일부만 제공되므로, 보이지 않는 세부 정보는 추측하지 말고, 보이는 범위 안에서만 분석해라.
"""
...
"""
        ...
        print("\n[GPT] GPT API를 이용해 유사도/기밀 유출 분석 리포트를 생성합니다...")
        llm_output = generate_with_gpt(args.gpt_model, full_prompt)

        if not llm_output:
            print("[경고] GPT에서 결과를 받지 못했습니다.")
            return
        ...
        if args.out_json:
            try:
                with open(args.out_json, "w", encoding="utf-8") as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                print(f"[저장] GPT 요약 포함 전체 결과 JSON 업데이트: {args.out_json}")
            except Exception as e:
        ...
        if args.out_summary:
            try:
                with open(args.out_summary, "w", encoding="utf-8") as f:
                    f.write(llm_output)
                print(f"[저장] GPT 마크다운 요약 파일: {args.out_summary}")
            except Exception as e:
