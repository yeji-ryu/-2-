    ...
    for r in results[:topn]:
        ...
        payload["top_matches"].append({
            ...
            "sentence_matches": [
                ...
                for m in (r.get("cosine_metrics", {}).get("matches", [])[:5])
            ]
        })
...
def summarize_with_llm_local(payload: Dict) -> str:
    """
    로컬 LLM(Ollama) qwen2.5:7b 모델을 사용해
    상위 3개 문서가 어떤 부분에서 유사한지 요약 생성
    """
    prompt = (
        "아래 JSON은 쿼리 문서와 상위 3개 문서의 문장 단위 매칭 결과입니다.\n"
        "각 문서에 대해 아래 형식으로 출력하세요.\n\n"
        "형식:\n"
        "문서명: XXX\n"
        "1) 쿼리 문장: \"...\"\n"
        "   상대 문장: \"...\"\n"
        ...
        "2) ...\n\n"
        "주의:\n"
        "- score가 높은 순서대로 상위 5개만 출력하세요.\n"
        "- 완전히 동일한 문장은 유사도 1.0로 간주\n"
        "- 의미적으로 매우 비슷한 문장(0.7 이상)도 포함\n\n"
        "JSON 데이터:\n"
...
def main():
    ...
    if args.command == "index":
        ...
        # ============================
        # 🔥 로컬 LLM(qwen2.5:7b) 요약 실행
        # ============================
        print("\n[LLM] 상위 3개 문서 유사 구간 요약 생성 중...")
...
# uv run python rag_pipeline_v4.py query --query_file "C:/Users/gkseh/hansung/HS/query_docs/외부의료.hwpx"