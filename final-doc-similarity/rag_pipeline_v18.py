...
def run_query(
    ...
    try:
        ...
        # cosine_rank_with_qdrant 의 반환값은 사용자 환경에 맞춰져 있다고 가정
    ...
    for doc_id in sorted(all_doc_ids):
        ...
        cosine_metrics = dense_detailed_results.get(doc_id) or {}
    ...
    out = {
        ...
        "query_sentences": chunks,  # 🔥 쿼리 문서 문장 리스트 추가
...
def build_llm_payload(result: Dict, topn: int = 3) -> Dict:
    ...
    for r in results[:topn]:
        ...
        # 🔥 문장 매칭 전체 리스트
        matches = r.get("cosine_metrics", {}).get("matches", [])

        # 🔥 코사인 유사도 높은 순 정렬
        matches_sorted = sorted(matches, key=lambda x: x.get("score", 0.0), reverse=True)
...
def summarize_with_llm_local(payload: Dict) -> str:
    ...
    1) 쿼리 문서 + 상위 3개 문서의 짧은 요약 (총 4개)
    2) 각 상위 문서별 문장 유사도 상위 5개 쌍
    를 생성
    """
    prompt = (
