...
try:
    ...
    import embedding
    import qdrant_dense
    import cosine  # <--- 문서 단위 코사인 (수정된 버전)
...
BM25_MAX_JSON = r"C:\Users\gkseh\hansung\HS\bm25_file_scores.json"
...
def run_query(
    ...
    # 2) Dense 검색 (cosine2에서 문서 단위 DocSim만 받음)
    ...
    try:
        ...
        _, all_dense_ranks = cosine.cosine_rank_with_qdrant(query_vectors, chunks)
...
# ============================
# 로컬 LLM(Ollama) 요약 함수 (마크다운 출력)
# ============================
def summarize_with_llm_local(payload: Dict) -> str:
    """
    payload를 기반으로 로컬 LLM을 호출해
    GitHub 마크다운 형식의 유사도 분석 리포트를 생성한다.
    """
    json_block = json.dumps(payload, ensure_ascii=False, indent=2)

    prompt = (
        "다음 JSON은 쿼리 문서와 상위 3개 문서의 문장 단위 매칭 결과입니다.\n"
        "이 정보를 이용해서, 아래 형식에 맞는 **GitHub 마크다운 문서**를 한국어로만 작성하세요.\n\n"
        "# 문서 유사도 분석 리포트\n\n"
        "## 1. 개요\n"
        "- 분석 대상: 쿼리 문서 1개와 상위 3개 유사 문서\n"
        "- 점수 기준: BM25, 코사인 유사도, 하이브리드 점수\n\n"
        "## 2. 문서 요약\n"
        "아래 형식을 반드시 지키세요.\n"
        "각 문서는 1~2문장으로 요약합니다.\n\n"
        "- 쿼리 문서(파일명: ...): ...\n"
        ...
        "- 내부 문서2(파일명: ...): ...\n"
        "- 내부 문서3(파일명: ...): ...\n\n"
        "요약 시 다음 내용을 포함하려고 노력하세요.\n"
        "- 핵심 의사결정이나 결론 (예: 투자 여부, 승인/반려, 리스크 요약 등)\n\n"
        "- 반드시 해당 문서에 대한 내용만 정리하세요"
        "## 3. 상위 문서 점수 요약\n"
        "- 표를 사용하여 각 상위 문서의 BM25%, 코사인%, 하이브리드%를 정리하세요.\n"
        "- 표 헤더 예시:\n"
        "  | 순위 | 문서명 | BM25(%) | 코사인(%) | 하이브리드(%) |\n\n"
        "## 4. 유사 문장 상세\n"
        "각 상위 문서별로 아래 형식을 반복해서 작성하세요. (문서당 최대 5개 쌍)\n\n"
        "### 문서명: 예시_문서명\n"
        "1. **쿼리 문장**  \n"
        "   \"...\"  \n"
        "   **상대 문서 문장**  \n"
        ...
        "2. ...\n\n"
        "작성 규칙:\n"
        "- 유사도 점수는 cosine_score를 사용하고, 내림차순(큰 값 → 작은 값)으로 정렬된 것처럼 표현하세요.\n"
        "- 의미적으로 많이 비슷해 보이는 문장(0.7 이상)은 꼭 포함하세요.\n"
        "- 전체 출력은 하나의 완전한 마크다운 문서가 되도록 작성하세요.\n"
        "- JSON 원문은 마크다운 문서에 다시 넣지 마세요.\n\n"
        "아래는 참고할 JSON 데이터입니다.\n\n"
        f"{json_block}\n"
    )
    ...
    try:
        ...
        if result.returncode != 0:
            return (
                f"[LLM 오류] 로컬 LLM 실행 실패 (코드 {result.returncode})\n"
                f"STDERR:\n{result.stderr}"
            )
        # LLM이 이미 마크다운 전체를 만들어 준다고 가정하고 그대로 반환
        return result.stdout.strip()
    except Exception as e:
        return f"[LLM 오류] 로컬 LLM 호출 실패: {e}"


def save_markdown_summary(markdown_text: str, query_doc_id: str,
                          out_dir: Path = BM25_BUILD_DIR) -> Path:
    """
    LLM에서 생성된 마크다운 리포트를 파일로 저장한다.
    파일명 예시: rag_summary_그린퓨처_투자보고서.md
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"rag_summary_{query_doc_id}.md"
    try:
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(markdown_text)
        print(f"[LLM] 마크다운 요약 리포트 저장 완료 → {out_path}")
    except Exception as e:
        print(f"[LLM 오류] 마크다운 저장 실패: {e}")
    return out_path
...
def main():
    ...
    if args.command == "index":
        ...
        # 3. 로컬 LLM(qwen2.5:7b) 요약 실행 (마크다운)
        print("\n[LLM] 상위 3개 문서 유사 구간 및 요약(마크다운) 생성 중...")
        summary_md = summarize_with_llm_local(payload)

        # 4. 마크다운 리포트 저장
        summary_path = save_markdown_summary(summary_md, query_doc_id, BM25_BUILD_DIR)

        # 5. CLI에도 마크다운 그대로 출력
        print("\n====== LLM 유사도 요약 (Markdown) ======")
        print(summary_md)
        print("====== LLM 유사도 요약 끝 ======")
        print(f"\n[INFO] 마크다운 리포트 파일 경로: {summary_path}")
        print(f"[INFO] LLM 입력 JSON 파일 경로: {payload_path}")
