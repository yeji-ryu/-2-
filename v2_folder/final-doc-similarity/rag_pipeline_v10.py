...
# BM25 절대 스케일 (BM25 ≈ BM25_SCALE일 때 bm25_rel ≈ 0.5)
BM25_SCALE = 1000.0
...
def run_query(
    ...
    for doc_id in sorted(all_doc_ids):
        ...
        # BM25: bm25_rel = bm25_raw / (bm25_raw + BM25_SCALE) (0~1)
        if bm25_raw > 0.0:
            bm25_rel = bm25_raw / (bm25_raw + BM25_SCALE)
        else:
            bm25_rel = 0.0
        ...
        # 하이브리드 = 0.7 * bm25_rel + 0.3 * cos_rel  (0~1)
        hybrid_score = BM25_WEIGHT * bm25_rel + COSINE_WEIGHT * cos_rel
        hybrid_pct = hybrid_score * 100.0

        results.append(
            {
                "doc_id": doc_id,
                "bm25_raw": bm25_raw,
                "bm25_rel": bm25_rel,          # 절대 BM25 비율 (0~1)
                "cosine_DocSim": cos_raw,
                "cosine_rel": cos_rel,         # 절대 Cosine 비율 (0~1)
