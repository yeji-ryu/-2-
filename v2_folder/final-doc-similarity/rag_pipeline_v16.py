...
try:
    ...
    print("[오류] google-generativeai 패키지가 없습니다. 'pip install google-generativeai' 후 다시 시도하세요.")
    sys.exit(1)
...
def load_doc_meta(build_dir: Path = BM25_BUILD_DIR) -> Dict[str, Dict[str, str]]:
    ...
    if not path.exists():
        print(f"[경고] doc_meta.json 없음: {path}")
        return {}

    try:
        data = json.load(open(path, "r", encoding="utf-8"))
    except Exception as e:
        print(f"[경고] doc_meta.json 로드 실패: {e}")
        return {}
...
def generate_with_gemini(model_name: str, prompt: str) -> str:
    """
    Google Gemini API 호출.
    환경 변수:
      - GOOGLE_API_KEY 또는 GEMINI_API_KEY 중 하나에 키가 들어 있어야 함.
    """
    api_key = os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
    if not api_key:
        print("[오류] GOOGLE_API_KEY 또는 GEMINI_API_KEY 환경변수가 설정되어 있지 않습니다.")
        return ""
    ...
    try:
        ...
        # 기본적으로 resp.text에 전체 텍스트가 들어 있음
        if hasattr(resp, "text") and resp.text:
            return resp.text.strip()
        # 혹시 없으면 candidates에서 꺼내기
...
def run_indexing(corpus_dir: Path, bm25_build_dir: Path):
    ...
    if not doc_paths:
        print(f"[경고] '{corpus_dir}'에 인덱싱할 문서가 없습니다.")
        return
    ...
    for fp in doc_paths:
        ...
        try:
            ...
            np.save(temp, vecs)
            qdrant_dense.append_npy_only(client_q, str(temp), file_id=doc_id)
            os.remove(temp)
        except Exception as e:
    ...
    # doc_meta 저장
    try:
        json.dump(doc_meta, open(DOC_META_JSON, "w", encoding="utf-8"),
    ...
    json.dump(all_lines, open(all_lines_json, "w", encoding="utf-8"),
              ensure_ascii=False)
...
def run_query(
    ...
    cosine_scores = {
        doc_id: r.get("DocSim", 0.0)
        for doc_id, r in dense_detailed_results.items()
    }
...
def main():
    parser = argparse.ArgumentParser(description="RAG Pipeline (Hybrid + Gemini 기밀 유출 리포트)")
    ...
    p_query.add_argument(
        ...
        type=str,
        default="",
        help="Gemini 요약 마크다운 저장 경로",
    )
    p_query.add_argument(
        "--gemini_model",
        type=str,
        default="gemini-1.5-flash",
        help="사용할 Gemini 모델 이름 (예: gemini-1.5-flash, gemini-1.5-pro 등)",
    )
    ...
    if args.command == "query":
        ...
        if not top3:
            print("[경고] 상위 문서가 없어 Gemini 리포트를 생성하지 않습니다.")
            return

        # 실제 문서 제목 3개
        top_ids = [r["doc_id"] for r in top3]
        while len(top_ids) < 3:
            top_ids.append(f"문서{len(top_ids)+1}")
        top1_id, top2_id, top3_id = top_ids[:3]
        ...
        if not internal_text_map:
            print("[경고] 내부 문서 텍스트가 없어 Gemini 리포트를 건너뜁니다.")
            return

        # 텍스트 truncate
        ext_text_trunc = truncate_text(query_text)

        # 프롬프트 헤더 (네가 원하는 스타일 반영)
        prompt_header = f"""
너는 "외부 문서와 내부 문서 TOP3 간 유사도 및 기밀 유출 위험"을 분석하는 한국어 보안 분석가다.

규칙:
- 비교는 외부 문서 vs 각 내부 문서(Top3)만 수행한다.
- 내부 문서끼리 비교하거나, 어느 회사가 더 투자매력도가 높다/낮다 같은 평가는 절대 하지 않는다.
- 요약은 반드시 본문 내용 기반의 3~5문장 자연어 단락이어야 하며, 목차 제목 나열만 하는 것은 금지한다.
- "유사 문장 예시"에 쓰는 문장은 반드시 아래 텍스트에 실제로 들어있는 문장을 그대로 인용해야 한다.
- 텍스트에 없는 구체적인 숫자, 지명, 인물, 리스크 내용을 새로 지어내지 말고, 보이지 않으면 "구체적인 수치는 잘리지 않아 보이지 않는다"처럼 설명하라.

출력 형식은 아래 예시와 최대한 비슷하게 맞춰라. (제목/구조는 유지하되, 내용은 네가 채운다.)

외부 문서 요약

(외부 문서가 어떤 회사/사업/투자 내용을 다루는지, 목적과 핵심 논지를 3~5문장으로 요약)

내부 문서 요약
{top1_id}

({top1_id} 문서가 어떤 회사/사업/투자 내용을 다루는지, 재무/리스크/밸류에이션 등 핵심 포인트를 3~5문장으로 요약)
...
(외부 문서와 {top1_id} 문서가 어떤 부분에서 비슷한지, 사업 구조/시장/재무/리스크/전략 관점에서 2~4문장으로 설명)
...
- 외부: "..."
  내부({top1_id}): "..."
(최대 5쌍, 실제 텍스트에 존재하는 문장만 사용할 것)

기밀 유출 위험 평가

(위 유사 문장을 근거로 기밀 유출 위험도를 "낮음/중간/높음" 중 하나로 평가하고, 어떤 내용이 문제가 될 수 있는지 2~4문장으로 설명)
...
이제 분석에 사용할 원문 일부를 제공한다.
각 문서의 내용 일부만 제공되므로, 보이지 않는 세부 정보는 추측하지 말고, 보이는 범위 안에서만 분석하라.
"""
...
"""
        ...
        print("\n[Gemini] Gemini API를 이용해 유사도/기밀 유출 분석 리포트를 생성합니다...")
        llm_output = generate_with_gemini(args.gemini_model, full_prompt)

        if not llm_output:
            print("[경고] Gemini에서 결과를 받지 못했습니다.")
            return
        ...
        if args.out_json:
            try:
                with open(args.out_json, "w", encoding="utf-8") as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                print(f"[저장] Gemini 요약 포함 전체 결과 JSON 업데이트: {args.out_json}")
            except Exception as e:
        ...
        if args.out_summary:
            try:
                with open(args.out_summary, "w", encoding="utf-8") as f:
                    f.write(llm_output)
                print(f"[저장] Gemini 마크다운 요약 파일: {args.out_summary}")
            except Exception as e:
                print(f"[경고] 요약 파일 저장 실패: {e}")

        # 콘솔에 최종 리포트 출력
        print("\n======================= Gemini 유사도/기밀 분석 리포트 =======================\n")
        print(llm_output)
        print("\n===========================================================================")
