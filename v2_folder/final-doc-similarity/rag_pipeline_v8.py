...
def normalize_scores(score_dict: Dict[str, float]) -> Dict[str, float]:
    """doc_id -> score 를 0~1로 min-max 정규화."""
    if not score_dict:
        return {}
...
def run_query(
    ...
    try:
        sparse_ranks_list = word.bm25_search_qdrant(
            ...
            tokenizer=BM25_TOKENIZER,
            topk=10000,  # 사실상 전체 다 가져오기 (내부에서 더 적게 줄 수도 있음)
        )
    ...
    # 하이브리드 = 0.7 * BM25 + 0.3 * Cosine
    results = []
    for doc_id in sorted(all_doc_ids):
        bm25_raw = bm25_scores.get(doc_id, 0.0)
        bm25_n = bm25_norm.get(doc_id, 0.0)
        cos_raw = cosine_scores.get(doc_id, 0.0)
        cos_n = cosine_norm.get(doc_id, 0.0)
        hybrid = BM25_WEIGHT * bm25_n + COSINE_WEIGHT * cos_n
        results.append(
            {
                "doc_id": doc_id,
                "bm25_raw": bm25_raw,
    ...
    # 최상위 문서를 기준으로 백분율 (0~100) 계산
    if results:
        top_score = results[0]["hybrid_score"]
        if top_score > 1e-12:
            for r in results:
                r["hybrid_pct"] = (r["hybrid_score"] / top_score) * 100.0
        else:
            for r in results:
                r["hybrid_pct"] = 0.0
    else:
