...
def normalize_by_max(score_dict: Dict[str, float]) -> Dict[str, float]:
    """
    doc_id -> score 를 0~1로 정규화.
    각 값 / (이 쿼리에서의 최대값).
    """
    if not score_dict:
        return {}
...
def run_query(
    ...
    try:
        ...
        # all_dense_ranks는 이미 전체 문서에 대한 결과라고 가정
    ...
    # 쿼리 기준 max BM25 / max Cosine으로 정규화
    bm25_rel = normalize_by_max(bm25_scores)
    cosine_rel = normalize_by_max(cosine_scores)

    results = []
    for doc_id in sorted(all_doc_ids):
        bm25_raw = bm25_scores.get(doc_id, 0.0)
        bm25_r = bm25_rel.get(doc_id, 0.0)     # 0~1
        cos_raw = cosine_scores.get(doc_id, 0.0)
        cos_r = cosine_rel.get(doc_id, 0.0)    # 0~1

        # 하이브리드 = 0.7 * BM25_rel + 0.3 * Cos_rel
        hybrid_score = BM25_WEIGHT * bm25_r + COSINE_WEIGHT * cos_r  # 0~1 범위

        results.append(
            {
                "doc_id": doc_id,
                "bm25_raw": bm25_raw,
                "bm25_rel": bm25_r,          # 쿼리 기준 BM25 비율
                "cosine_DocSim": cos_raw,
                "cosine_rel": cos_r,         # 쿼리 기준 Cosine 비율
                "hybrid_score": hybrid_score,
                "hybrid_pct": hybrid_score * 100.0,  # 0~100%
    ...
    if args.command == "index":
        ...
        for i, r in enumerate(results[:topk], 1):
            ...
            print(
                ...
                f"(BM25={bm25_raw:.1f}, Cosine={cos_raw:.4f})"
            )
